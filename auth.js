...
passport.use('patient', new LocalStrategy(async function verify(username, password, cb) {
    ...
    await querypatient.findOne().then(function (user) {
        ...
        cb(null, false, { status: 401, message: 'Patient Does Not Exist' })
    });
}));

passport.use('doctor', new LocalStrategy(async function verify(username, password, cb) {
    const querydoctor = Doctor.where({ username: username });
    await querydoctor.findOne().then(function (user) {
        if (user.password === password) {
            cb(null, user, { status: 200, message: 'Login Successful.' });
        }
        else {
        ...
        cb(null, false, { status: 401, message: 'Doctor Does Not Exist' })
    });
}));
